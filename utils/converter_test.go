...
func TestConvertClipboardToConfluenceContent(t *testing.T) {
	tests := []struct {
		name string
		json string
		want *response.ConfluenceContent
	}{
		{
			name: "Success convert to confluence content",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_ = clipboard.WriteAll(tt.json)
			if got := ConvertClipboardToConfluenceContent(); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ConvertClipboardToConfluenceContent() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestConvertConfluenceContentToCreateTestCase(t *testing.T) {
	type args struct {
		content     *response.ConfluenceContent
		authorLogin string
	}
	tests := []struct {
		name string
		args args
		want []*request.CreateTestCase
	}{
		{
			name: "Success convert confluenceContent to createTestCase request",
			args: args{
				content: &response.ConfluenceContent{
					Id:     "162592801",
					Type:   "page",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := ConvertConfluenceContentToCreateTestCase(tt.args.content, tt.args.authorLogin); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ConvertConfluenceContentToCreateTestCase() = %v, want %v", got, tt.want)
			}
		})
	}
}

func Test_createTestCaseStep(t *testing.T) {
	type args struct {
		number          int
		step            string
		expectedResults string
	}
	manualExecutionType := testlink.ExecutionTypeManual
	tests := []struct {
		name string
		args args
		want *testlink.TestCaseStep
	}{
		{
			name: "Success create testCaseStep",
			args: args{
				number:          1,
				step:            "the steps",
				expectedResults: "the results",
			},
			want: &testlink.TestCaseStep{
				Number:          1,
				Actions:         "the steps",
				ExpectedResults: "the results",
				ExecutionType:   manualExecutionType.Value(),
			},
		},
		{
			name: "Success create testCaseStep, empty step",
			args: args{
				number:          2,
				step:            "",
				expectedResults: "the results",
		...
		{
			name: "Success create testCaseStep, empty result",
			args: args{
				number:          1,
				step:            "the steps",
				expectedResults: "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := createTestCaseStep(tt.args.number, tt.args.step, tt.args.expectedResults); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("createTestCaseStep() = %v, want %v", got, tt.want)
			}
		})
	}
}
